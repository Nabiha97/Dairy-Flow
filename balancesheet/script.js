...
const mapped = apiData.map(c => ({
    ...
    date: String(c.updated_at || '').substring(0, 10) || new Date().toISOString().split('T')[0],
    litres: { 
        mtd:   parseFloat(c.litres_mtd   || 0), 
        today: parseFloat(c.litres_today || 0), 
        total: parseFloat(c.litres_total || 0) 
    },
    sales: { 
        mtd:   parseFloat(c.sales_mtd   || 0), 
        today: parseFloat(c.sales_today || 0), 
        total: parseFloat(c.sales_total || 0) 
    },
    paid: { 
        mtd:   parseFloat(c.cash_mtd   || 0), 
        today: parseFloat(c.cash_today || 0), 
        total: parseFloat(c.cash_total || 0) 
    },
    credit: { 
        mtd:   parseFloat(c.credit_mtd   || 0), 
        today: parseFloat(c.credit_today || 0), 
        total: parseFloat(c.credit_total || 0) 
    }
}));
    customers = mapped;
    allCustomers = [...mapped];
